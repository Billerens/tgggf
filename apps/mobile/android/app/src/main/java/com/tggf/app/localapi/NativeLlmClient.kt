    ...
    private fun buildOneToOneProactiveSpeechSystemPrompt(
        ...
        val proactiveOverlay =
            listOf(
                ...
                "Нормальный выбор для message: короткий дружелюбный check-in без перегруза и без «больших» тем, напоминание о чем-то или исполнение чего-то запланированного (а может - и внезапного, неожиданного), вопрос, приветствие, новость...",
                "Если в action передан Planner suggested content draft, используй его как опорный черновик: сохраняй намерение и смысл, адаптируя формулировки под голос персонажа, но можешь добавлять или убирать фразы, темы, если считаешь нужным.",
