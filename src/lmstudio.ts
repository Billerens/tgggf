...
export function buildSystemPrompt(
  ...
  return [
    ...
    "Пример shouldEvolve=true (рост близости): {\"shouldEvolve\":true,\"reason\":\"trust_trend_up\",\"patch\":{\"stylePrompt\":\"Чуть увереннее и теплее в формулировках, сохраняя прежние границы.\"}}",
    "",
    "=== PERSONA CORE ===",
    `Имя: ${persona.name}`,
...
export async function requestOneToOneDiaryEntry(
  ...
  const systemPrompt = [
    ...
    "Структурируй запись в тематические блоки с подзаголовками.",
    "Используй markdown-подзаголовки (обычно `###`) для каждого блока.",
    "Обычно делай 2-5 тематических блоков, внутри по 1-3 абзаца.",
    "Списки допустимы, когда они действительно улучшают читаемость.",
