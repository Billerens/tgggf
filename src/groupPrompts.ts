...
export function buildGroupPersonaSystemPrompt({
  ...
  return [
    ...
    "- запрещено утверждать, что изображение уже отправлено/прикреплено/приложено, если в ответе нет блока <comfyui_image_description>;",
    "- запрещены сценические ремарки об отправке контента в *звездочках* (например: *прикрепила фото*, *скинула фотку*);",
    "- если изображения нет в служебном блоке <comfyui_image_description>, считай что изображения НЕТ: не упоминай его как будто оно отправлено;",
    "- если ты отказываешь в фото или фото не требуется, не добавляй <comfyui_image_description> и не упоминай отправку изображения в тексте реплики;",
    ...
    "- если изображение действительно нужно, ОБЯЗАТЕЛЬНО добавь после реплики ОДИН или НЕСКОЛЬКО отдельных блоков (если отправляешь несколько изображений, то и блоков будет несколько):",
    "<comfyui_image_description>",
    "Подробное визуальное описание кадра: кто в кадре, поза, эмоция, одежда/материалы, фон, свет, ракурс, композиция, настроение.",
    "</comfyui_image_description>",
    "- внутри comfyui_image_description: только визуальные детали, без markdown и без пояснений;",
    ...
    "- если в тексте есть утверждение «фото/картинка отправлена», а блока <comfyui_image_description> нет — перепиши ответ;",
    "- если в тексте есть упоминание НЕСКОЛЬКИХ изображений, а блоков <comfyui_image_description> меньше — перепиши ответ;",
    "- если есть ремарки в *...* про отправку фото — перепиши ответ;",
    "- если нет явного запроса на изображение, а ты добавила <comfyui_image_description> — убери блок;",
