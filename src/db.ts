...
interface TgGfDb extends DBSchema {
  ...
  diaryEntries: {
    key: string;
    value: DiaryEntry;
    indexes: { "by-chat": string; "by-createdAt": string; "by-updatedAt": string };
  };
  messages: {
    key: string;
    value: ChatMessage;
...
function normalizeDiaryEntry(entry: DiaryEntry): DiaryEntry {
  const createdAt = toTrimmedString(entry.createdAt) || new Date().toISOString();
  const updatedAt = toTrimmedString(entry.updatedAt) || createdAt;
  const sourceRangeRaw =
    entry.sourceRange && typeof entry.sourceRange === "object"
      ? entry.sourceRange
      : { messageCount: 0 };
  const messageCountRaw =
    typeof sourceRangeRaw.messageCount === "number" &&
    Number.isFinite(sourceRangeRaw.messageCount)
      ? sourceRangeRaw.messageCount
      : 0;
  return {
    ...entry,
    id: toTrimmedString(entry.id) || crypto.randomUUID(),
    chatId: toTrimmedString(entry.chatId),
    personaId: toTrimmedString(entry.personaId),
    markdown: (entry.markdown || "").trim(),
    tags: normalizeDiaryTags(entry.tags),
    sourceRange: {
      fromMessageId: toTrimmedString(sourceRangeRaw.fromMessageId) || undefined,
      toMessageId: toTrimmedString(sourceRangeRaw.toMessageId) || undefined,
      fromCreatedAt: toTrimmedString(sourceRangeRaw.fromCreatedAt) || undefined,
      toCreatedAt: toTrimmedString(sourceRangeRaw.toCreatedAt) || undefined,
      messageCount: Math.max(0, Math.floor(messageCountRaw)),
    },
    ...
    dbPromise = openDB<TgGfDb>(DB_NAME, DB_VERSION, {
      upgrade(db) {
        ...
        if (!db.objectStoreNames.contains("diaryEntries")) {
          const diaryEntries = db.createObjectStore("diaryEntries", {
            keyPath: "id",
          });
          diaryEntries.createIndex("by-chat", "chatId");
          diaryEntries.createIndex("by-createdAt", "createdAt");
          diaryEntries.createIndex("by-updatedAt", "updatedAt");
        }
...
export const dbApi = {
  ...
  async deletePersona(personaId: string) {
    ...
    for (const chat of chats) {
      ...
      const diaryEntryKeys = await tx
        .objectStore("diaryEntries")
        .index("by-chat")
        .getAllKeys(chat.id);
      for (const key of diaryEntryKeys) {
        await tx.objectStore("diaryEntries").delete(key);
      }
  ...
  async getDiaryEntries(chatId: string) {
    const db = await getDb();
    const rows = await db.getAllFromIndex("diaryEntries", "by-chat", chatId);
    const normalized = rows.map((row) => normalizeDiaryEntry(row));
    await Promise.all(normalized.map((row) => db.put("diaryEntries", row)));
    return normalized.sort((a, b) => b.createdAt.localeCompare(a.createdAt));
  },

  async getAllDiaryEntries() {
    const db = await getDb();
    const rows = await db.getAll("diaryEntries");
    const normalized = rows.map((row) => normalizeDiaryEntry(row));
    await Promise.all(normalized.map((row) => db.put("diaryEntries", row)));
    return normalized.sort((a, b) => b.createdAt.localeCompare(a.createdAt));
  },
  ...
  async saveDiaryEntry(entry: DiaryEntry) {
    const db = await getDb();
    await db.put("diaryEntries", normalizeDiaryEntry(entry));
  },
  ...
  async saveDiaryEntries(entries: DiaryEntry[]) {
    ...
    for (const entry of entries) {
      await tx.store.put(normalizeDiaryEntry(entry));
    }
    await tx.done;
  },

  async deleteDiaryEntries(ids: string[]) {
    ...
    for (const entryId of uniqueIds) {
      await tx.store.delete(entryId);
    }
    await tx.done;
  },

  async getDiaryMomentsByTags(
    chatId: string,
    tags: string[],
    limit = 20,
  ): Promise<DiaryMoment[]> {
    const normalizedRequestedTags = normalizeDiaryTags(tags, 64);
    if (normalizedRequestedTags.length === 0) return [];
    const db = await getDb();
    const rows = await db.getAllFromIndex("diaryEntries", "by-chat", chatId);
    const entries = rows
      .map((row) => normalizeDiaryEntry(row))
      .sort((a, b) => b.createdAt.localeCompare(a.createdAt));
    const requestedSet = new Set(normalizedRequestedTags);
    const nowMs = Date.now();
    const scored = entries
      .map((entry) => {
        const matches = entry.tags.filter((tag) => requestedSet.has(tag));
        ...
        const score = matches.length * 10 + recencyBoost;
        const snippetSource = entry.markdown.replace(/\s+/g, " ").trim();
        const snippet =
          snippetSource.length > 220
            ? `${snippetSource.slice(0, 219).trimEnd()}…`
            : snippetSource;
        return {
          entryId: entry.id,
          chatId: entry.chatId,
      ...
      .filter((item): item is DiaryMoment => Boolean(item));
    return scored
      .sort((a, b) => {
