...
## 7.7 Mature Tone and Safety Boundaries (18+)
...
Жесткие границы безопасности:
1. Сексуальные сцены допускаются только при взаимном согласии совершеннолетних участников в контексте сцены.
2. Non-consensual sexual content не генерируется.
3. Если narrative пытается уйти в non-consensual 18+:
   - арбитр обязан остановить/перевести сцену,
   - зафиксировать событие риска,
   - предложить безопасный narrative-исход без явного сексуального насилия.
4. Насилие/угроза как элемент мрачного сюжета могут присутствовать, но без сексуализированного принуждения.
...
- [ ] Runtime-валидатор границ mature-контента.
...
- [ ] Mature-границы соблюдаются автоматически в runtime.
...
2. Риск: race conditions в async генерациях.
   - Мера: `turnId` validation на каждом этапе + per-chat lock.
...
5. Риск: уход narrative в неприемлемый mature-контент.
   - Мера: runtime validator + hard stops + fallback narration policy.
